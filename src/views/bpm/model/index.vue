        ...
        <el-card class="app-row">
            ...
            <el-form class="-mb-15px" :model="queryParams" ref="queryFormRef" :inline="true" label-width="68px">
                ...
                <el-form-item label="流程分类" prop="category" >
                    <el-select v-model="queryParams.category" placeholder="请选择流程分类" clearable style="min-width: 200px;">
                        <el-option v-for="category in categoryList" :key="category.code" :label="category.name"
                            :value="category.code" />
                    </el-select>
                </el-form-item>
                <el-form-item>
                    ...
                    <el-button type="primary" plain @click="openForm('新建流程')" v-hasPermi="['bpm:model:create']">
                        <Icon icon="ep:plus"  /> 新建流程
                    </el-button>
                    <el-button type="success" plain @click="openImportForm" v-hasPermi="['bpm:model:import']">
                        <Icon icon="ep:upload" /> 导入流程
                    </el-button>
                </el-form-item>
            </el-form>
        </el-card>
        ...
        <el-card class="app-row">
            <el-table v-loading="loading" :data="list">
                ...
                <el-table-column label="表单信息" align="center" prop="formType" width="200">
                    <template #default="scope">
                        <el-button v-if="scope.row.formType === 10" type="primary" link
                            @click="handleFormDetail(scope.row)">
                            <span>{{ scope.row.formName }}</span>
                        </el-button>
                        <el-button v-else-if="scope.row.formType === 20" type="primary" link
                            @click="handleFormDetail(scope.row)">
                    ...
                    <el-table-column label="激活状态" align="center" prop="processDefinition.version" width="85">
                        <template #default="scope">
                            <el-switch v-if="scope.row.processDefinition"
                                v-model="scope.row.processDefinition.suspensionState" :active-value="1"
                                :inactive-value="2" @change="handleChangeState(scope.row)" />
                        </template>
                    </el-table-column>
                ...
                <el-table-column label="操作" align="center" width="240" fixed="right">
                    <template #default="scope">
                        <el-button link type="primary" @click="openForm('修改流程', scope.row.id)"
                        ...
                        <el-button link type="primary" @click="handleSimpleDesign(scope.row.id)"
                            v-hasPermi="['bpm:model:update']">
                            仿钉钉设计流程
                        </el-button>
