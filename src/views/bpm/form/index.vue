    ...
    <ContentWrap>
      ...
      <el-form ref="queryFormRef" :inline="true" :model="queryParams" class="-mb-15px" label-width="68px">
        <el-form-item label="表单名" prop="name">
          <el-input v-model="queryParams.name" class="!w-240px" clearable placeholder="请输入表单名"
      ...
      <el-table v-loading="loading" :data="list">
        ...
        <el-table-column align="center" label="状态" prop="status">
          <template #default="scope">
            <dict-tag :type="DICT_TYPE.SYS_COMMON_STATUS" :value="scope.row.status" />
          </template>
        </el-table-column>
...
const getList = async () => {
  loading.value = true
  try {
    const data = await FormApi.getFormPage(queryParams).then(res => res.data)
    if (!data) return
